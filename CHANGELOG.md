...
### Fixed
- **Pet art centering preserves internal alignment.** v0.6.1 centered
  each art line independently, which scrambled sprites whose lines had
  different widths (after `rstrip`). The fix strips the common
  leading-whitespace prefix from all art lines and centers the block
  as a whole. Adults that previously clipped on the right (snake,
  dragon) now fit cleanly.
